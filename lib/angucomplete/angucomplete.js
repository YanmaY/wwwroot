...
angular.module('angucomplete', [] )
    ...
    return {
                    ...
                    if ($scope.localData) {
                        ...
                        $http.get($scope.url + str, {}).
                            success(function(responseData, status, headers, config) {
                                $scope.searching = false;
                                $scope.processResults((($scope.dataField) ? responseData[$scope.dataField] : responseData.Data ), str);
                            }).
        ...
        <div class="angucomplete-holder">\
				<input id="{{id}}_value" ng-model="searchStr" type="text" placeholder="{{placeholder}}" class="{{inputClass}}" onmouseup="this.select();" ng-focus="resetHideResults()" ng-blur="hideResults()" AUTOCOMPLETE="OFF" />\
				<div id="{{id}}_dropdown" class="angucomplete-dropdown" ng-if="showDropdown">\
					<div class="angucomplete-searching" ng-show="searching">正在搜索...</div>\
					<div class="angucomplete-searching" ng-show="!searching && (!results || results.length == 0)">没有找到匹配的记录</div>\
