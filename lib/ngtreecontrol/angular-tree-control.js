            ...
            return {
                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function ($scope, $templateCache, $interpolate, treeConfig) {
                    ...
                    function defaultIsLeaf(node) {
                        ...
                        return node.leaf; //修改源码，将是否为页节点的判断改为由leaf属性判断
                    }
                    ...
                    function defaultIsSelectable(node) {
                        return node.selectable == undefined ? true : node.selectable;
                    }

                    $scope.options = $scope.options || {};
                    ensureDefault($scope.options, "multiSelection", false);
                    //ensureDefault($scope.options, "nodeChildren", "children");
                    ensureDefault($scope.options, "nodeChildren", "Data"); //修改源码，将默认子节点的属性改为Data.
                    ...
                    if (!template) {
                        template =
                            '<ul {{options.ulClass}} >' +
                            '<li ng-repeat="node in node.{{options.nodeChildren}} |filter:itemCode| filter:filterExpression:filterComparator {{options.orderBy}}" ng-class="headClass(node)" {{options.liClass}}' +
                            'set-node-to-data>' +
