...
(function () {
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            ...
            if (list) {//去掉图片初始宽度
                //if (editor.options.initImgWidth) {
                //    for (var i = 0; i < list.length; i++) {
                //        var orgImgWidth = list[i].width;
                //        var orgImgHeight = list[i].height;
                //        if (!list[i].width)
                //            list[i].width = editor.options.initImgWidth
                //        else {
                //            list[i].width = Math.min(list[i].width, editor.options.initImgWidth);
                //            list[i].height = orgImgHeight * list[i].width / orgImgWidth;
                //        }
                //    }
                //}
    ...
    UploadImage.prototype = {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        json.width = file._info.width;
                        json.height = file._info.height;
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                list.push({
                    src: prefix + data.url + '?width=600&height=450&mode=max',  //--多图上传时候添加缩略图参数
                    _src: prefix + data.url + '?width=600&height=450&mode=max',//--多图上传时候添加缩略图参数
                    title: data.title,
                    alt: data.original,
                    floatStyle: align,
                    //width: data.width,
                    //height:data.height  图片宽高不设置，使用缩略图参数控制图片
                });
            }
            return list;
        }
